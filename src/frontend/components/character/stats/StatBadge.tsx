...
export const StatBadge = ({ icon, label, children, reverse }: StatBadgeProps) => (
  <div className="group/badge hover:border-character-muted/80 hover:bg-character-surface/10 hover:shadow-character-muted/30 m-0 flex items-center justify-center gap-1 rounded-full border border-zinc-800/50 bg-zinc-950/40 px-2 py-1.5 backdrop-blur-md transition-all duration-300 hover:shadow-[0_0_15px_var(--character-muted)] md:gap-2 md:px-4 md:py-2">
    <div className="text-character-flare flex-shrink-0 transition-all duration-300 group-hover/badge:brightness-125">
      {icon}
    </div>
    <div className={cn('flex items-baseline gap-1 md:gap-2', reverse && 'flex-row-reverse')}>
      {label && (
