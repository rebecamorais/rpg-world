...
- [x] Abstrair o estado global complexo do Personagem, como reações na UI, em um Contexto ou Contextos menores (`useReducer`). *(Resolvido via custom hook `useCharacterEditor.ts`)*
...
- [x] Separar a Rota `/` (Home) para ser uma Landing Page convidativa ao invés de forçar o login direto.
- [x] Mover todo o processo de entrada para a rota dedicada `/login`.
- [x] Inserir campo de "Senha" no `LoginForm.tsx` (mesmo aceitando qualquer valor temporariamente).
